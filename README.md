...
riss {
    scanPackages.set(listOf("no.example.api"))
    paths.set(listOf("/api/**"))
}
```

Declare document metadata on a dedicated type. Existing swagger annotations on
controllers and DTOs are read as input.
